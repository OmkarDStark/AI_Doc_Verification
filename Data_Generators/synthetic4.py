...
def draw_and_label_fields(image_path):
    ...
    while True:
        ...
        if field_type == "number":
            ...
            lang = "en"
            if subtype == "address":
                lang = input("Language for address? [en/mr/both] (default: en): ").strip().lower()
            if lang not in ["en", "mr", "both"]:
                lang = "en"
            field_generators[field_name] = {"type": "text", "subtype": subtype, "lang": lang}

        elif field_type == "name":
            lang = input("Language for name? [en/mr/both] (default: en): ").strip().lower()
            if lang not in ["en", "mr", "both"]:
                lang = "en"
            field_generators[field_name] = {"type": "name", "lang": lang}
        else:
...
def create_synthetic_doc(image_path, fields, font_sizes, generators, list_data, output_path="synthetic_output.jpg"):
    ...
    for label, (x1, y1, x2, y2) in fields.items():
        ...
        if isinstance(value, dict) and "en" in value and "mr" in value:
            draw.text((x1 + 5, y1 + 2), value["en"], font=font, fill="black")
            draw.text((x1 + 5, y1 + 2 + font_size), value["mr"], font=font, fill="black")
        else:
